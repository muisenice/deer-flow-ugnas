...
如果你是在 x86_64 / Intel 的绿联 NAS 上为单个可信操作者做局域网内部署，并希望保持和上游生产 Compose 路径一致，请直接参考 [backend/docs/UGREEN_NAS.md](backend/docs/UGREEN_NAS.md)。该指南基于官方 `docker/docker-compose.yaml` 和 `scripts/deploy.sh`，并明确说明何时需要额外导出 `DEER_FLOW_CONFIG_PATH` 与 `DEER_FLOW_EXTENSIONS_CONFIG_PATH`，不需要额外维护 NAS 专用 Compose 分支。
